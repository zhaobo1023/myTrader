...
class BacktestEngine:
    ...
    def __init__(self, config: BacktestConfig, risk_config=None):
        """
        初始化回测引擎

        Args:
            config: 回测配置
            risk_config: 风控配置（可选），传入则启用风控引擎
        """
        config.validate()
        self.config = config
        ...
        for _, signal in day_signals.iterrows():
            ...
            if self.risk_engine is not None:
                if stock_code in price_data:
                    ...
                    ohlcv_df = stock_df[stock_df.index <= date]

                ctx = self.risk_engine.build_context_from_portfolio(
                    stock_code=stock_code,
                    price=price,
                    date=date,
                ...
                risk_decision = self.risk_engine.evaluate(ctx)

                if not risk_decision.approved:
                    continue

                # 应用仓位缩放建议
                weight = weight * risk_decision.suggested_position_pct

            # 计算仓位大小
            if (self.risk_engine is not None
                    and self.config.position_sizing == 'risk_parity'):
                from risk_manager import calc_atr_position
                if ohlcv_df is None and stock_code in price_data:
                    stock_df = price_data[stock_code]
                    ohlcv_df = stock_df[stock_df.index <= date]
                sizing = calc_atr_position(
                    price=price,

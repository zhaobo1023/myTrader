    ...
    <div style={{
      ...
      <div>
        <div style={{ display: 'flex', alignItems: 'center', gap: '6px' }}>
          ...
          <span style={{ fontSize: '14px', fontWeight: 600, color: 'var(--text-primary)' }}>{cn.name}</span>
...
function DashboardStockTable({ stocks, onStockClick }: { stocks: any[]; onStockClick: (code: string) => void }) {
  ...
  return (
    <div style={{ overflowX: 'auto' }}>
      <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '12px' }}>
        <thead>
          <tr>
            <th style={{ padding: '8px 6px', fontSize: '11px', fontWeight: 600, color: 'var(--text-secondary)', textAlign: 'left', borderBottom: '1px solid var(--border-subtle)' }}>标的</th>
            ...
            <th style={{ padding: '8px 6px', fontSize: '11px', fontWeight: 600, color: 'var(--text-secondary)', textAlign: 'center', borderBottom: '1px solid var(--border-subtle)', width: 60 }}>操作</th>
          </tr>
        </thead>
        <tbody>
          {sorted.map((s: any) => {
            ...
            return (
              <tr
                key={s.stock_code}
                onClick={() => isClickable && onStockClick(s.stock_code)}
                style={{
              ...
              >
                ...
                <td style={{ padding: '8px 6px', textAlign: 'right', fontWeight: 600, color: sc.total_score != null && sc.total_score >= 60 ? COLOR_UP : 'var(--text-primary)' }}>{scoreStr(sc.total_score)}</td>
...
export default function ThemeDashboardTab({ themeId, onStockClick }: { themeId: number; onStockClick: (code: string) => void }) {
  ...
  const { summary, mappings, stocks } = data;
  ...
  return (
    <div>
      ...
      {/* Market Split Bar */}
      <div style={{ display: 'flex', gap: '2px', borderRadius: '4px', overflow: 'hidden', height: '6px', marginBottom: '20px' }}>
      ...
      <div>
        ...
        <div style={{ background: 'var(--bg-panel)', border: '1px solid var(--border-subtle)', borderRadius: '8px', overflow: 'hidden' }}>
          <DashboardStockTable stocks={stocks} onStockClick={onStockClick} />
        </div>
      </div>
    </div>
  );

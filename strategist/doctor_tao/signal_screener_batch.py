# -*- coding: utf-8 -*-
"""
陶博士策略 - 信号筛选器（内存优化版）

实现分批处理，避免一次性加载所有股票数据导致 OOM
"""
import pandas as pd
import numpy as np
from datetime import datetime, timedelta
from typing import List, Dict, Optional
from dataclasses import dataclass
import os
import sys
import gc

ROOT = os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__))))
sys.path.insert(0, ROOT)

from strategist.doctor_tao.data_fetcher import DoctorTaoDataFetcher
from strategist.doctor_tao.indicators import IndicatorCalculator
from config.db import execute_query


@dataclass
class ScreenerParams:
    """筛选参数配置"""
    # 基本面过滤参数
    min_list_days: int = 250
    min_avg_amount: float = 5000
    min_net_profit: float = 0
    min_price: float = 3.0

    # 动量筛选参数
    rps_threshold: float = 95
    rps_continuous_weeks: int = 4
    rps_continuous_min: float = 90
    return_60d_rank_threshold: float = 70
    volume_ratio_threshold: float = 1.2

    # 反转候选参数
    reversal_price_percentile_threshold: float = 35
    reversal_return_60d_threshold: float = 0.15
    reversal_rps_slope_threshold: float = 1.0
    reversal_volume_ratio_threshold: float = 1.5
    reversal_rps_max: float = 90

    # 大盘条件参数
    index_code: str = '000001.SH'

    # 分批处理参数（新增）
    batch_size: int = 500  # 每批处理的股票数量


class SignalScreenerBatch:
    """信号筛选器（内存优化版 - 分批处理）"""

    def __init__(self, params: Optional[ScreenerParams] = None):
        self.params = params or ScreenerParams()
        self.fetcher = DoctorTaoDataFetcher(use_cache=True)

    def apply_prefilter(self, stock_list: List[str], filter_table: pd.DataFrame) -> List[str]:
        """基本面底线过滤"""
        print(f"\n[1/4] 基本面过滤...")
        print(f"  输入股票数: {len(stock_list)}")

        today = datetime.now().date()

        # 1. 非ST股票
        filtered = filter_table[filter_table['is_st'] == False].copy()
        print(f"  非ST股票: {len(filtered)} 只")

        # 2. 上市满1年
        min_list_natural_days = int(self.params.min_list_days * 365 / 250)
        filtered = filtered[
            (pd.to_datetime(filtered['list_date']).dt.date < today - timedelta(days=min_list_natural_days))
        ]
        print(f"  上市满{self.params.min_list_days}交易日: {len(filtered)} 只")

        # 3. 成交额过滤
        if 'avg_amount_60d' in filtered.columns:
            filtered = filtered[filtered['avg_amount_60d'] >= self.params.min_avg_amount]
            print(f"  成交额≥{self.params.min_avg_amount}万: {len(filtered)} 只")

        # 4. 净利润过滤
        if 'latest_net_profit' in filtered.columns:
            filtered = filtered[filtered['latest_net_profit'] > self.params.min_net_profit]
            print(f"  净利润>0: {len(filtered)} 只")

        # 只保留在输入列表中的股票
        filtered = filtered[filtered['stock_code'].isin(stock_list)]

        result = filtered['stock_code'].tolist()
        print(f"  过滤后股票数: {len(result)} 只")
        print(f"  过滤率: {(1 - len(result)/len(stock_list))*100:.1f}%")

        return result

    def apply_price_filter(self, indicators_df: pd.DataFrame) -> pd.DataFrame:
        """价格过滤：收盘价 ≥ 3元"""
        if self.params.min_price > 0:
            before_count = len(indicators_df)
            indicators_df = indicators_df[indicators_df['close'] >= self.params.min_price]
            print(f"  价格≥{self.params.min_price}元: {len(indicators_df)} 只（过滤 {before_count - len(indicators_df)} 只）")
        return indicators_df

    def screen_momentum(self, indicators_df: pd.DataFrame) -> pd.DataFrame:
        """第一层：动量筛选"""
        print(f"\n[2/4] 动量筛选...")

        filtered = indicators_df.copy()
        initial_count = len(filtered)

        # 1. RPS ≥ 95
        filtered = filtered[filtered['rps'] >= self.params.rps_threshold]
        print(f"  RPS >= {self.params.rps_threshold}: {len(filtered)} 只")

        # 2. 股价站上月线
        filtered = filtered[filtered['close'] > filtered['ma20']]
        print(f"  股价 > MA20: {len(filtered)} 只")

        # 3. 股价站上年线
        if 'ma250' in filtered.columns:
            filtered = filtered[filtered['close'] > filtered['ma250']]
            print(f"  股价 > MA250: {len(filtered)} 只")

        # 4. 近60日涨幅排名前30%
        if 'return_60d_rank' in filtered.columns:
            filtered = filtered[filtered['return_60d_rank'] >= self.params.return_60d_rank_threshold]
            print(f"  60日涨幅排名前30%: {len(filtered)} 只")

        # 5. 成交量放大
        if 'volume_ratio' in filtered.columns:
            filtered = filtered[filtered['volume_ratio'] >= self.params.volume_ratio_threshold]
            print(f"  成交量放大: {len(filtered)} 只")

        filtered['signal_type'] = 'momentum'
        print(f"  最终动量信号: {len(filtered)} 只")

        return filtered

    def screen_reversal(self, indicators_df: pd.DataFrame) -> pd.DataFrame:
        """第二层：反转候选筛选"""
        print(f"\n[3/4] 反转候选筛选...")

        filtered = indicators_df.copy()
        initial_count = len(filtered)

        # 1. 价格历史低位
        filtered = filtered[filtered['price_percentile'] < self.params.reversal_price_percentile_threshold]
        print(f"  价格分位 < {self.params.reversal_price_percentile_threshold}%: {len(filtered)} 只")

        # 2. RPS尚未到顶
        filtered = filtered[filtered['rps'] < self.params.reversal_rps_max]
        print(f"  RPS < {self.params.reversal_rps_max}: {len(filtered)} 只")

        # 3. RPS快速提升
        if 'rps_slope' in filtered.columns:
            filtered = filtered[filtered['rps_slope'] > self.params.reversal_rps_slope_threshold]
            print(f"  RPS斜率Z > {self.params.reversal_rps_slope_threshold}: {len(filtered)} 只")

        # 4. 成交量异动
        if 'volume_ratio' in filtered.columns:
            filtered = filtered[filtered['volume_ratio'] >= self.params.reversal_volume_ratio_threshold]
            print(f"  成交量异动: {len(filtered)} 只")

        filtered['signal_type'] = 'reversal'
        print(f"  最终反转候选: {len(filtered)} 只")

        return filtered

    def check_market_condition(self) -> Dict[str, any]:
        """大盘条件判断"""
        print(f"\n[4/4] 大盘条件判断...")

        result = {
            'status': 'bullish',
            'close': None,
            'ma50': None,
            'ma120': None,
            'message': ''
        }

        try:
            index_df = self.fetcher.fetch_daily_price(
                self.params.index_code,
                start_date='2023-01-01'
            )

            if len(index_df) == 0:
                print("  无法获取大盘数据，默认通过")
                result['message'] = '无法获取大盘数据'
                return result

            # 计算 MA50/MA120
            index_df['ma50'] = index_df['close'].rolling(window=50).mean()
            index_df['ma120'] = index_df['close'].rolling(window=120).mean()

            latest = index_df.iloc[-1]
            close = float(latest['close'])
            ma50 = float(latest['ma50']) if pd.notna(latest['ma50']) else None
            ma120 = float(latest['ma120']) if pd.notna(latest['ma120']) else None

            result['close'] = close
            result['ma50'] = ma50
            result['ma120'] = ma120

            print(f"  上证指数: {close:.2f}")
            print(f"  MA50: {ma50:.2f if ma50 else 'N/A'}, MA120: {ma120:.2f if ma120 else 'N/A'}")

            if ma50 is None or ma120 is None:
                result['message'] = 'MA数据不足'
                return result

            if close > ma50:
                result['status'] = 'bullish'
                result['message'] = '沪指站上MA50'
                print(f"  状态: BULLISH")
            elif close > ma120:
                result['status'] = 'neutral'
                result['message'] = '沪指在MA50与MA120之间'
                print(f"  状态: NEUTRAL")
            else:
                result['status'] = 'bearish'
                result['message'] = '沪指跌破MA120'
                print(f"  状态: BEARISH")

            return result

        except Exception as e:
            print(f"  大盘条件判断失败: {e}，默认通过")
            result['message'] = f'判断失败: {e}'
            return result

    def _process_batch(
        self,
        stock_batch: List[str],
        start_date: str
    ) -> Optional[pd.DataFrame]:
        """
        处理一批股票的信号筛选

        Args:
            stock_batch: 股票代码批次
            start_date: 数据开始日期

        Returns:
            筛选结果 DataFrame（只包含最新日期的数据）
        """
        # 1. 批量获取价格数据
        price_dict = self.fetcher.fetch_daily_price_batch(
            stock_batch,
            start_date=start_date
        )

        if not price_dict:
            return None

        # 2. 转换为 DataFrame（只保留必要的列）
        price_list = []
        for code, df in price_dict.items():
            if len(df) > 0:
                df = df.copy()
                df['stock_code'] = code
                # 只保留必要的列以减少内存
                cols_to_keep = ['stock_code', 'trade_date', 'open', 'high', 'low', 'close', 'volume', 'amount']
                df = df[cols_to_keep]
                price_list.append(df)

        if not price_list:
            return None

        # 3. 合并价格数据
        price_df = pd.concat(price_list, ignore_index=True)

        # 4. 计算指标
        try:
            indicators_df = IndicatorCalculator.calc_all_indicators(price_df)
        except Exception as e:
            print(f"  批次指标计算失败: {e}")
            return None

        # 5. 只保留最新日期
        latest_date = indicators_df['trade_date'].max()
        latest_df = indicators_df[indicators_df['trade_date'] == latest_date].copy()

        # 6. 价格过滤
        latest_df = self.apply_price_filter(latest_df)

        # 7. 动量筛选
        momentum_df = self.screen_momentum(latest_df)

        # 8. 反转候选筛选
        reversal_df = self.screen_reversal(latest_df)

        # 9. 合并结果
        result_list = []
        if len(reversal_df) > 0:
            result_list.append(reversal_df)
        if len(momentum_df) > 0:
            result_list.append(momentum_df)

        if result_list:
            result = pd.concat(result_list, ignore_index=True)
            return result
        else:
            return None

    def run_screener(
        self,
        date: Optional[str] = None,
        output_csv: bool = True
    ) -> pd.DataFrame:
        """
        运行筛选器（分批处理版本）

        Args:
            date: 指定日期，格式 'YYYY-MM-DD'，默认最新日期
            output_csv: 是否输出CSV文件

        Returns:
            筛选结果DataFrame
        """
        print("=" * 60)
        print("陶博士策略 - 信号筛选器（内存优化版）")
        print("=" * 60)

        start_date = (datetime.now() - timedelta(days=750)).strftime('%Y-%m-%d')

        # 1. 获取所有股票列表
        print("\n[1/5] 获取股票列表...")
        all_stocks = self.fetcher.fetch_all_stocks()
        print(f"  总股票数: {len(all_stocks)}")

        # 2. 获取基本面过滤表
        print("\n[2/5] 获取基本面过滤表...")
        filter_table = self.fetcher.fetch_filter_table()

        # 3. 基本面过滤
        filtered_stocks = self.apply_prefilter(all_stocks, filter_table)

        if not filtered_stocks:
            print("没有通过基本面过滤的股票")
            return pd.DataFrame()

        # 4. 分批处理价格数据和筛选
        print(f"\n[3/5] 分批处理价格数据（批次大小: {self.params.batch_size}）...")
        print(f"  待处理股票数: {len(filtered_stocks)}")
        print(f"  预计批次数: {(len(filtered_stocks) + self.params.batch_size - 1) // self.params.batch_size}")

        all_results = []
        batch_num = 0

        for i in range(0, len(filtered_stocks), self.params.batch_size):
            batch_num += 1
            batch_stocks = filtered_stocks[i:i + self.params.batch_size]

            print(f"\n  批次 {batch_num}: 处理 {len(batch_stocks)} 只股票...")

            try:
                batch_result = self._process_batch(batch_stocks, start_date)

                if batch_result is not None and len(batch_result) > 0:
                    print(f"    找到 {len(batch_result)} 只信号股票")
                    all_results.append(batch_result)
                else:
                    print(f"    无信号")

                # 及时释放内存
                del batch_result
                gc.collect()

            except Exception as e:
                print(f"    批次处理失败: {e}")
                continue

        # 5. 合并所有批次结果
        if not all_results:
            print("\n没有找到任何信号股票")
            return pd.DataFrame()

        print(f"\n[4/5] 合并筛选结果...")
        result = pd.concat(all_results, ignore_index=True)
        print(f"  合并后信号数: {len(result)}")

        # 删除批次结果以释放内存
        del all_results
        gc.collect()

        # 6. 去重（同一股票可能在不同批次中出现）
        if len(result) > 0:
            # 对于同一股票，如果同时有动量和反转信号，保留动量信号
            result = result.sort_values(
                ['stock_code', 'signal_type'],
                ascending=[True, False]  # signal_type: 'momentum' > 'reversal'
            ).drop_duplicates(subset=['stock_code'], keep='first')
            print(f"  去重后信号数: {len(result)}")

        # 7. 大盘条件判断
        market_condition = self.check_market_condition()
        market_status = market_condition['status']

        # 8. 根据大盘状态过滤信号
        if market_status == 'bearish':
            print("\n大盘处于熊市状态，暂停所有信号输出")
            return pd.DataFrame()
        elif market_status == 'neutral':
            if 'rps' in result.columns:
                before_count = len(result)
                result = result[result['rps'] >= 97]
                print(f"大盘中性，只保留RPS≥97: {before_count} -> {len(result)}")

        # 9. 添加大盘状态
        if len(result) > 0:
            result['market_status'] = market_status
            result['market_message'] = market_condition['message']

        # 10. 查询股票名称
        if len(result) > 0:
            print("\n[5/5] 查询股票名称...")
            name_map = self._fetch_stock_names(result['stock_code'].tolist())
            result['stock_name'] = result['stock_code'].map(name_map)
            cols = list(result.columns)
            cols.remove('stock_name')
            idx = cols.index('stock_code') + 1
            cols.insert(idx, 'stock_name')
            result = result[cols]

        # 11. 输出 CSV
        if output_csv and len(result) > 0:
            output_dir = os.path.join(ROOT, 'output', 'doctor_tao')
            os.makedirs(output_dir, exist_ok=True)

            output_file = os.path.join(
                output_dir,
                f"signals_{datetime.now().strftime('%Y%m%d_%H%M%S')}.csv"
            )

            result.to_csv(output_file, index=False, encoding='utf-8-sig')
            print(f"\n结果已保存到: {output_file}")

        # 12. 打印统计信息
        print("\n" + "=" * 60)
        print("筛选结果统计:")
        print("=" * 60)
        if len(result) > 0:
            momentum_count = sum(1 for s in result.get('signal_type', []) if s == 'momentum')
            reversal_count = sum(1 for s in result.get('signal_type', []) if s == 'reversal')
            print(f"动量信号: {momentum_count} 只")
            print(f"反转候选: {reversal_count} 只")
            print(f"大盘状态: {market_status.upper()} - {market_condition['message']}")
        else:
            print("无信号股票")
        print("=" * 60)

        return result

    def _fetch_stock_names(self, codes: List[str]) -> Dict[str, str]:
        """从数据库查询股票名称"""
        if not codes:
            return {}
        try:
            placeholders = ','.join(['%s'] * len(codes))
            rows = execute_query(
                f"SELECT stock_code, stock_name FROM trade_stock_basic WHERE stock_code IN ({placeholders})",
                tuple(codes),
            )
            if rows:
                result = {r['stock_code']: r['stock_name'] for r in rows}
                found = sum(1 for v in result.values() if v)
                print(f"  查询到 {found}/{len(codes)} 只股票名称")
                return result
        except Exception as e:
            print(f"  查询股票名称失败: {e}")
        return {}


# 便捷函数
def run_screener_batch(date: Optional[str] = None, output_csv: bool = True) -> pd.DataFrame:
    """运行分批筛选器"""
    screener = SignalScreenerBatch()
    return screener.run_screener(date=date, output_csv=output_csv)


if __name__ == '__main__':
    # 测试筛选器
    result = run_screener_batch()
    if len(result) > 0:
        print("\n前10只股票:")
        cols = ['stock_code', 'stock_name', 'signal_type', 'rps', 'close', 'ma20', 'ma250']
        available_cols = [c for c in cols if c in result.columns]
        print(result[available_cols].head(10))

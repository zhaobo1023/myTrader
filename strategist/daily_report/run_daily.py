...
5. 数据日期处理：若技术面数据标注了[数据日期=YYYY-MM-DD，非今日]，则分析时必须写明"截至YYYY-MM-DD数据显示..."，严禁用"今日"或"昨日"等模糊表达
6. 整体概览中如有股票存在陈旧数据，需在开头注明"部分股票行情数据非今日，以下分析以实际数据日期为准"
"""
...
def _format_stock_block(p: dict, tech_map: dict, ann_map: dict, today: date) -> list[str]:
    ...
    if t.get('close') is not None:
        close = t['close']
        data_date = t.get('trade_date', '')
        # 检查数据是否陈旧（不是报告日期的数据）
        if data_date and str(data_date) != today.isoformat():
            data_stale = True
            tech_parts.append(f'[数据日期={data_date}，非{today.isoformat()}，数据可能陈旧]')
        else:
            tech_parts.append(f'数据日期={data_date}')
...
def _build_prompt(user_info: dict, tech_map: dict, ann_map: dict, today: date) -> str:
    ...
    missing_codes = [
        p['stock_code'] for p in positions
        if p['stock_code'] not in tech_map
    ]

...
def load_factor_panel(start_date: str, end_date: str) -> pd.DataFrame:
    ...
    sql_quality_tpl = f"""
        ...
        WHERE calc_date = '__trade_date__'
    """
    logger.info("  Loading quality_factor...")
    df_quality = _read_sql_by_batches(sql_quality_tpl, 'trade_date', dates)

    # 5) 每日基本面: dv_ttm
    sql_daily_basic_tpl = f"""
        SELECT stock_code, trade_date,
               dv_ttm
        FROM trade_stock_daily_basic
        WHERE trade_date = '__trade_date__'
    """
    logger.info("  Loading daily_basic...")
    df_daily_basic = _read_sql_by_batches(sql_daily_basic_tpl, 'trade_date', dates)

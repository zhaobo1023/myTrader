...
├── docs/                      # 文档与设计稿
│
├── web/                       # Next.js 16 前端 (TypeScript + Tailwind)
...
│   │   ├── rag/               # RAG 研报问答
│   │   ├── admin/             # 用户管理后台
│   │   ├── public/screener/   # 公开筛股页 (Freemium)
│   │   └── (auth)/            # 登录/注册
│   ├── src/components/        # React 组件
│   ├── src/hooks/             # 自定义 Hooks
│   ├── src/lib/               # API client, auth, store, query
...
│   ├── test.yml               # lint + build + pytest
│   └── deploy.yml             # SSH 部署到 ECS
│
├── docker-compose.yml         # Docker 编排 (Redis + API + Nginx)
├── Dockerfile                 # API 服务镜像
├── nginx.conf                 # Nginx 反代配置
├── Makefile                   # dev/build/migrate 命令
│
...
基于 `docs/myTrader_tech_plan.md` 技术方案，完成 28 个任务中的 27 个（T28 生产上线需服务器环境）。

...
列出3-5条今日最重要的客观事实/数据变化。每条标注来源。
格式: - **[来源]** 事实描述（必须有数字）

### 核心观点（按价值排序）

列出3-5条最有价值的观点。价值排序: 有数据支撑 > 有逻辑推演 > 纯观点。
格式: 编号. **[看多/看空/中性] 观点** -- 展开说明, 标注来源和置信度
如果多家一致观点, 合并并注明"(多家一致)"

### 板块信号

按板块归类机会和风险, 只列有逻辑或数据支撑的:
- **板块名**: 机会/风险说明
...
1. 不编造数据, 所有数字必须来自原文
2. 观点必须标注来源(公众号名)
3. 区分事实(客观陈述)和观点(主观判断), 事实放第一个section
4. 如果多个来源说同一件事, 只保留信息最丰富的那个
...
async def generate_curated_report(
    ...
    rows = execute_query(
        """SELECT title, source_name, article_type, grade, digest_json, summary
    ...
    _send_card(
        title=title,
        verdict='{}篇A/B级文章精选'.format(len(rows)),
        doc_url=doc['url'],
    )

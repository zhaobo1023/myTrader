...
## 规则（强制，不得违反）
1. 对标记为 [MISSING]、[STALE] 或 [ANOMALY] 的指标，声明"数据不可用"或"数据滞后"，绝不可编造数值，也不可引用该数值进行任何分析
2. 绝不可编造具体数字（成交量、涨跌家数、指数涨跌幅、资金流向）——如果数据中没有提供，就不要写
3. 如果某个维度的所有核心指标均缺失，该维度输出"[数据不足，未产出]"
...
8. 报告末尾必须附"数据说明"，格式：数据日期: 大盘信号=YYYY-MM-DD, 全球资产=YYYY-MM-DD, ETF偏离=YYYY-MM-DD, 涨跌停=YYYY-MM-DD, 恐贪=YYYY-MM-DD。对于滞后或异常的数据源，额外标注"[滞后]"或"[异常]"
9. 若某数据区标记为[数据缺失]或[ANOMALY]，对应章节直接输出"[数据不足，未产出]"，不得使用过期或异常数据生成分析
...
给出明确的操作节奏建议（如"上午观察量能再决定是否操作"），不要给出具体仓位建议。"""
...
关注尾盘跳水信号、盘后利空消息、外盘盘前动向，给出风险等级判断，不要给出具体仓位建议。"""
...
INDICATOR_NAMES = {
    ...
    'spgsci': 'S&P商品指数(GSCI)',
    'spgsci_energy': 'GSCI能源', 'spgsci_pm': 'GSCI贵金属',
    'spgsci_ag': 'GSCI农产品', 'spgsci_livestock': 'GSCI畜牧', 'spgsci_softs': 'GSCI软商品',
    'nat_gas': '天然气(USD)', 'copper': '铜(USD)', 'silver': '白银(USD)',
    'wheat': '小麦(USc)', 'corn': '玉米(USc)', 'soybean': '大豆(USc)',
...
def _is_date_current(data_date_str: str, target_date: date) -> bool:
    """Check if data_date is current (within 1 day of target_date).

    Allows <=1 day gap to handle weekends/holidays where the latest
    trading day is the previous business day.
    """
    if not data_date_str or data_date_str == '?':
        return False
    try:
        from datetime import datetime as _dt
        data_dt = _dt.strptime(str(data_date_str), '%Y-%m-%d').date()
        return (target_date - data_dt).days <= 1
    except Exception:
        return False
...
def _collect_dashboard_snapshot() -> tuple:
    """
    Collect A-share dashboard signal data from the market-overview API.

    Returns:
        (formatted_text, data_date_str | None)
    """
    try:
        ...
        return '(大盘信号数据不可用)', None

    if not data:
        return '(大盘信号数据不可用)', None

    data_date = data.get('data_date')  # e.g. '2026-04-16'
    ...
    if temp.get('available'):
        ...
        # If volume is anomalous, show warning instead of misleading numbers
        if vol.get('anomaly') or vr.get('anomaly'):
            lines.append('  成交额: [ANOMALY: 数据异常，不可引用] | 量/MA20: [ANOMALY]')
        else:
            vol_suffix = ' (数据: {})'.format(data_date) if not data.get('is_fresh') and data_date else ''
            lines.append('  成交额: {}亿{} | 量/MA20: {} ({}) | 换手分位: {}%'.format(
                vol.get('value', '-'), vol_suffix,
                vr.get('value', '-'), vr.get('signal', '-'), tp.get('value', '-')))
    ...
    if macro.get('available'):
        ...
        date_suffix = ' (数据: {})'.format(data_date) if not data.get('is_fresh') and data_date else ''
        lines.append('**宏观背景** ({}){}'.format(macro.get('level_label', '-'), date_suffix))
...
def _collect_etf_log_bias_snapshot() -> tuple:
    ...
    data_date = str(trade_date) if trade_date != '?' else None
...
def _collect_limit_stock_snapshot(trade_date: str = None) -> tuple:
...
def _collect_fear_greed_snapshot() -> tuple:
    ...
    data_date = td if td != '?' else None

    text = '日期: {} | 恐贪指数: {}/100 ({}) | VIX: {} ({}) | 风险提示: {}'.format(
...
async def generate_briefing(session: str = 'morning') -> dict:
    ...
    # --- 1. Collect macro data (already has per-row freshness tags) ---
    indicators = MORNING_INDICATORS if session == 'morning' else EVENING_INDICATORS
    snapshot, freshness_stats = _collect_data_snapshot(indicators, target_date=today_date)

    # --- 2. Collect A-share dashboard ---
    dashboard_snapshot, dashboard_date = _collect_dashboard_snapshot()

    # --- 3. Collect ETF log bias ---
    etf_log_bias_snapshot, etf_date = _collect_etf_log_bias_snapshot()

    # --- 4. Collect limit-up/down details ---
    limit_stock_snapshot, limit_date = _collect_limit_stock_snapshot()
    ...
    data_sources = {
        ...
        'etf_log_bias': {'label': 'ETF偏离', 'date': etf_date, 'current': False},
        'limit_stock': {'label': '涨跌停', 'date': limit_date, 'current': False},
        'fear_greed': {'label': '恐贪', 'date': fear_date, 'current': False},
    }
    for key in ['dashboard', 'etf_log_bias', 'limit_stock', 'fear_greed']:
        src = data_sources[key]
        src['current'] = _is_date_current(src['date'], today_date) if src['date'] else False

    # Replace stale sections with placeholder
    stale_sections = []
    if not data_sources['dashboard']['current']:
        stale_sections.append(('数据区1', data_sources['dashboard']))
        dashboard_snapshot = '[数据缺失: 大盘信号, 最新数据日期: {}]'.format(
            dashboard_date or '无')
    if not data_sources['etf_log_bias']['current']:
        stale_sections.append(('数据区3', data_sources['etf_log_bias']))
        etf_log_bias_snapshot = '[数据缺失: ETF偏离度, 最新数据日期: {}]'.format(
            etf_date or '无')
    if not data_sources['limit_stock']['current']:
        stale_sections.append(('数据区4', data_sources['limit_stock']))
        limit_stock_snapshot = '[数据缺失: 涨跌停明细, 最新数据日期: {}]'.format(
            limit_date or '无')
    if not data_sources['fear_greed']['current']:
        stale_sections.append(('数据区5', data_sources['fear_greed']))
        fear_greed_snapshot = '[数据缺失: 恐贪指数, 最新数据日期: {}]'.format(
            fear_date or '无')
    ...
    structured_data = {
        ...
        'data_sources': {k: {'date': v['date'], 'current': v['current']} for k, v in data_sources.items()},
    ...
    if stale_sections:
        declaration_lines = [
            '',
            '### 数据完整性声明',
            '以下数据区缺失当日数据，对应分析章节请输出"[数据不足]"，不得引用陈旧数据：',
        ]
        for section_id, src in stale_sections:
            declaration_lines.append('- {}: {} (最新: {})'.format(
                section_id, src['label'], src['date'] or '无'))

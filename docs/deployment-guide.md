...
2. **前端使用蓝绿部署，全程无停机**
3. **API 使用 Gunicorn HUP reload，准零停机**
...
[Nginx :443]
       |
       +-- /api/*  --> mytrader-api:8000  (Gunicorn, HUP reload)
       |
...
1. 运行测试（`test.yml`）
2. 测试通过后，`scp deploy_remote.sh` 到服务器并执行
...
[2] API HUP reload
    -> 发 SIGHUP 给 gunicorn master
    -> 等待 /health 恢复（最多 30s）
    -> 失败则 docker restart mytrader-api（最多 40s）
